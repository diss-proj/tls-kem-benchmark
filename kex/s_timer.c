...
SSL* do_tls_handshake(SSL_CTX* ssl_ctx)
{
    ...
    // Any data will be sent as soon as possible
...
int main(int argc, char* argv[])
{
    ...
    // Session considered to be shutdown, no close_notify alert sent to peer 
    ...
    ret = SSL_CTX_load_verify_locations(ssl_ctx, "/opt/httpd/cacert/CA.crt", 0);
    if(ret != 1)
    {
        fprintf(stderr, "6");
        goto ossl_error;
